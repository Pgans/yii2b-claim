...
GROUP BY k.SUB_FUND
            ...
            INNER JOIN m_sum_subfund s ON s.tran_id = m.tran_id AND s.ITEM_SOURCE = 'M_REGISTER'
            INNER JOIN inscl_nhso i ON i.CODE = m.MAININSCL
            WHERE m.DATEADM BETWEEN '25621001' AND '25630131' 
            GROUP BY s.SUB_FUND
                     ")->queryAll(); 
        ...
        INNER JOIN m_sum_subfund s ON s.tran_id = m.tran_id AND s.ITEM_SOURCE = 'M_REGISTER'
        INNER JOIN inscl_nhso i ON i.CODE = m.MAININSCL
        WHERE m.DATEADM BETWEEN '25630201' AND '25630531' 
        GROUP BY s.SUB_FUND
         ")->queryAll(); 
    ...
    for($i=0;$i<sizeof($trimas2);$i++){
       ...
       # $cnt[] = $d['cnt']*1;//นำไปคูณ 1 ป้องกันเป็น string
        ...
        INNER JOIN m_sum_subfund s ON s.tran_id = m.tran_id AND s.ITEM_SOURCE = 'M_REGISTER'
        INNER JOIN inscl_nhso i ON i.CODE = m.MAININSCL
        WHERE m.DATEADM BETWEEN '25630601' AND '25630930' 
        GROUP BY s.SUB_FUND
         ")->queryAll(); 
                            ...
                            $dataProvider3 = new \yii\data\ArrayDataProvider([
                                ...
                                'pagination' => [
                                 ...
                                 return $this->render('trimas', [
                                     ...
                                     't3dataProvider'=> $f1dataProvider,
                                     't3dataProvider'=> $f2dataProvider,
                                     'sql1'=> $sql1,
                                     'name'=> $name ,
                                     'amount'=>$amount ,
                                     'adjrw'=>$adjrw ,
